...
func (cc *CommandController) IssueGetCommandByName(w http.ResponseWriter, r *http.Request) {
	...
	// Query params
	pushEvent := utils.ParseQueryStringToString(r, v2.PushEvent, v2.ValueNo)
	returnEvent := utils.ParseQueryStringToString(r, v2.ReturnEvent, v2.ValueYes)

	var response interface{}
	var statusCode int

	event, err := application.IssueReadCommandByName(deviceName, commandName, pushEvent, returnEvent, cc.dic)
	if err != nil {
		lc.Error(err.Error(), clients.CorrelationHeader, correlationId)
		lc.Debug(err.DebugMessages(), clients.CorrelationHeader, correlationId)
		response = commonDTO.NewBaseResponse("", err.Message(), err.Code())
